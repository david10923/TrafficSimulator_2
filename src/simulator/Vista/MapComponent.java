...
public class MapComponent extends JPanel implements TrafficSimObserver {
	...
	private void drawRoads(Graphics g) {
		for (Road r : _map.getRoads()) {
			...
			if (idx != -1 && r.equals(r.getDest().getIncomingRoadList().get(idx))) {
			...
			int roadColorValue = 200 - (int) (200.0 * Math.min(1.0, (double) r.getGlobal_Pollution() / (1.0 + (double) r.getMasive_Pollution())));
